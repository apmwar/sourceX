  ...
  <body>
    ...
    <?php
      ...
      if(isset($_POST['submit']) && ($_POST['password'] == $_POST['passwordagain'])){
        ...
        $form = $_POST;
        $sql = "INSERT INTO students (fname, lname, dob, dept, year, section, cgpa, id, email, mobi, password) VALUES
        (:fname, :lname, :dob, :dept, :year, :section, :cgpa, :id, :email, :mobi, :password)";
        $query = $db->prepare( $sql );
        $query->execute( array( ':fname'=>$givenfname, ':lname'=>$givenlname, ':dob'=>$givendob, ':dept'=>$givendept, ':year'=>$givenyear, ':section'=>$givensection, ':cgpa'=>$givencgpa, ':id'=>$givenid, ':email'=>$givenemail, ':mobi'=>$givenmobi, ':password'=>$givenpassword));
      }
    ?>
  </body>
</html>
